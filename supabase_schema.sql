...
create table if not exists projects (
  ...
  user_id         uuid, -- تمت الإضافة لدعم الحسابات
  name            text not null,
...
-- أوامر تحديث الجداول لو كانت موجودة مسبقاً (عشان تضمن إن كل حاجة تمام)
alter table projects add column if not exists user_id uuid;
alter table scans add column if not exists user_id uuid;
alter table asset_history add column if not exists user_id uuid;
alter table js_dependencies add column if not exists user_id uuid;
